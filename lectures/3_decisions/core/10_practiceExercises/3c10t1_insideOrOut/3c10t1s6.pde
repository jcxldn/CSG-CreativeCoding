...
void setup() {
  ...
  // random(0,5) -> 0-4.9999 
  x = random(rWidth/2, width-(rWidth/2)); // random(high); or random(low, high);
  y = random(cDiameter/2, height-(cDiameter/2)); // instead of rHeight as cDiamater is larger
}
...
void draw() {
  ...
  if (!inRectBound()) {
    ...
    if (dist(x, y, mouseX, mouseY) <= cDiameter/2) { // -25 -> 25
